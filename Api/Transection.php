...
function register_transections($conn){
    ...
    $query = "CALL sp_transections('','$title','$org','$type','$date','$number','$name','$memo', '$split', '$category', '$amount')";
...
function update_transections($conn){
    ...
    // Building the query and calling the stored procedure
    $query = "CALL sp_transections('$id','$title','$org','$type','$date','$number','$name','$memo', '$split', '$category', '$amount')";
...
function get_transections($conn){
    ...
    $data = array();
    $array_data = array();
    $query = "SELECT * FROM `transection` ORDER BY id DESC LIMIT 5;";
    $result = $conn->query($query);
...
function get_transection_report($conn){
    ...
    $data = array();
    $array_data = array();
    $query = "CALL rp_transection('$org','$from','$to')";
    $result = $conn->query($query);
