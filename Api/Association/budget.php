...
function register_budget($conn){
    ...
    $query = "CALL sp_budget('','$semId','$description','$amount','$associationName')";
...
function update_budget($conn){
    ...
    $associationName = $_SESSION['AssociationName'];
    // Building the query and calling the stored procedure
    $query = "CALL sp_budget('$id','$semId','$description','$amount','$associationName')";
...
function get_budget($conn) {
    $data = array();
    $array_data = array();
    $query = "SELECT * FROM seminar_budget_items WHERE `AssociationName` = '{$_SESSION['AssociationName']}'";
    $result = $conn->query($query);
