...
function register_Jobs($conn){
    ...
    $query = "CALL sp_jobs('','$jobTitle','$associationName')";
...
function update_jobs($conn){
    ...
    $query = "CALL sp_jobs('$id','$jobTitle', '$associationName')";
...
function get_jobs($conn){
    ...
    $data = array();
    $array_data = array();
    $query = "SELECT * FROM `jobs` WHERE `AssociationName` = '{$_SESSION['AssociationName']}'";
    $result = $conn->query($query);
